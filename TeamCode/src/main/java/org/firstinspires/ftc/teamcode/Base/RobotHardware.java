...
public abstract class RobotHardware extends RobotBaseV1 {
    ...
    // ===== EXPERIMENTAL FIELD-CENTRIC ====
    /**
     * Field-Centric Drive - robot moves relative to the field, not itself
     * Left stick controls direction (relative to driver), right stick controls rotation
     */
    protected void FieldCentricDrive() {
        // Get joystick inputs
        double x = gamepad1.left_stick_x;      // Strafe (positive = right)
        double y = -gamepad1.left_stick_y;     // Forward (negative because gamepad is inverted)
        double turn = gamepad1.right_stick_x;  // Rotation

        // Get the robot's current heading from IMU
        double botHeading = Math.toRadians(getGlobal());

        // Rotate the movement direction based on robot heading
