...
public abstract class AutonomousBaseV1 extends RobotHardware {
    ...
    /**
     * Sets the lift servo to a specific position
     * @param position servo position (0.0 to 1.0)
     */
    protected void setLiftPosition(double position) {
        lift.setPosition(position);
        telemetry.addLine("Lift set to position: " + position);
        telemetry.update();
    }

    /**
     * Moves lift to position and waits
     * @param position servo position (0.0 to 1.0)
     * @param waitTime seconds to wait after moving
     */
    protected void moveLift(double position, double waitTime) {
        setLiftPosition(position);
        waitSec(waitTime);
    }

    /**
     * Sets sortOne and sortTwo servo positions
     * @param sortOnePosition position for sortOne servo (0.0 to 1.0)
     * @param sortTwoPosition position for sortTwo servo (0.0 to 1.0)
     */
    protected void setSortPosition(double sortOnePosition, double sortTwoPosition) {
        sortOne.setPosition(sortOnePosition);
        sortTwo.setPosition(sortTwoPosition);
        telemetry.addLine("Sort servos set - sortOne: " + sortOnePosition + ", sortTwo: " + sortTwoPosition);
        telemetry.update();
    }

    /**
     * Moves sort servos and waits
     * @param sortOnePosition position for sortOne servo
     * @param sortTwoPosition position for sortTwo servo
     * @param waitTime seconds to wait after moving
     */
    protected void moveSort(double sortOnePosition, double sortTwoPosition, double waitTime) {
        setSortPosition(sortOnePosition, sortTwoPosition);
        waitSec(waitTime);
    }

    /**
     * Activates left sort configuration (based on teleop left bumper)
     */
    protected void sortLeft() {
        setSortPosition(1, 0);
    }

    /**
     * Activates right sort configuration (based on teleop right bumper)
     */
    protected void sortRight() {
        setSortPosition(0, 1);
    }
    /**
     * Sets lift servo to middle position (0.5)
     */
    protected void liftMiddle() {
        setLiftPosition(0.5);
    }
    ///////////////////////////////////
    ///////////////Gyro////////////////
